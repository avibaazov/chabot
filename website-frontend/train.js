document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('tag').addEventListener('input', toggleTrainButtonBasedOnInputs);
    document.getElementById('add-pattern').addEventListener('click', () => addDynamicField('patterns-wrapper', 'patterns'));
    document.getElementById('add-response').addEventListener('click', () => addDynamicField('responses-wrapper', 'responses'));
...
function fetchExistingTrainingData(botId) {
    ...
    .then(data => {
        if (data.trainingData && data.trainingData.length > 0) {
            ...
            document.querySelector('.existing-tags-container').style.display = 'flex';
        } else {
            document.querySelector('.existing-tags-container').style.display = 'none';
        }
    })
...
function areInputFieldsEmpty() {
    ...
    return tag === '' && patternInputs.every(input => input === '') && responseInputs.every(input => input === '');
}
...
function displayExistingTrainingData(trainingDataArray, clearExisting = true) {
    const existingTagsContainer = document.querySelector('.existing-tags-container');
    const trainButton = document.getElementById('train');
    if (clearExisting) {
    ...
    existingTagsContainer.querySelectorAll('.delete-training-btn').forEach(button => {
        button.addEventListener('click', deleteTrainingData);
    });
    existingTagsContainer.style.display = trainingDataArray.length > 0 ? 'flex' : 'none';
...
function deleteTrainingData(e) {
    ...
    allTrainingData = allTrainingData.filter(trainingData => trainingData.id !== idToDelete);
...
function addTrainingData(e) {
    ...
    if (tag && patterns.length && responses.length) {
        ...
        const existingTrainingData = allTrainingData.find(data => data.tag === tag);

        if (existingTrainingData) {
            // Add patterns and responses to the existing tag
            existingTrainingData.patterns = Array.from(new Set([...existingTrainingData.patterns, ...patterns]));
            existingTrainingData.responses = Array.from(new Set([...existingTrainingData.responses, ...responses]));
            displayExistingTrainingData(allTrainingData);
        } else {
            // Add new training data
            const newTrainingData = { id: generateUniqueId(), tag, patterns, responses };
...
function trainBot(e) {
    e.preventDefault();
    if (!confirm("Are you sure you want to retrain your bot? This might take some time.")) {
        return;
    }
